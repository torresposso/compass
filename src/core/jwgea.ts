...
import { MODERN_SIGN_RULERS, modernRulerOfLongitude, signOfLongitude } from "./rulership.js";
import { JwgeaAnalysis } from "./Schema.js";

/**
 * Compute the Jeffrey Wolf Green Evolutionary Astrology (JWGEA) analysis for a
 * natal chart.
 *
 * Pure function over a Caelus {@link Chart}: derives the Pluto Polarity Point
 ...
 *
 * Throws when Pluto or the true node are unavailable for the chart instant,
 * since both are required for a complete evolutionary analysis.
 */
export function computeJwgea(chart: Chart): JwgeaAnalysis {
  ...
  if (!trueNode) {
    throw new Error("JWGEA requires the true nodal axis, which is missing from this chart.");
  }

  const plutoPolarityPoint = (pluto.lon + 180) % 360;

  const northNodeSign = trueNode.sign;
  const northNodeRuler = MODERN_SIGN_RULERS[northNodeSign];
  if (northNodeRuler === undefined) {
    throw new Error(`No modern ruler defined for sign "${northNodeSign}"`);
  }
  ...
  const skippedSteps: Array<string> = [];
  for (const [id, body] of Object.entries(chart.bodies)) {
    ...
    if (distFromSquare <= SQUARE_ORB && !skippedSteps.includes(id)) {
